...
import { UseScrollPositionExample } from './use-scroll-position.tsx';
...
import { useScrollPosition } from '@adambrgmn/hooks';

const FadeInOnPosition = ({ breakpoint, children }) => {
  const { y } = useScrollPosition(fn => () => window.requestAnimationFrame(fn));
