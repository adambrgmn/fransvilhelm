    ...
    <ul>
      {tasks.map(task => (
        <li key={task.id}>
          ...
          {task.state === AsyncState.fulfilled && <ShowData data={task.value} />}
          {task.state === AsyncState.rejected && <ShowError error={task.error} />}
        </li>
      ))}
    </ul>
  );
...
| opts.onAllSettled        | `(tasks: Task[]) => void` | false    | -       | Callback fired once all tasks ar either fullfilled or rejected                           |
| opts.onFullfilled        | `(task: Task) => void`    | false    | -       | Callback fired once for every fullfiled task                                             |
| opts.onRejected          | `(task: Task) => void`    | false    | -       | Callback fired once for every rejected task                                              |
...
| task.value | `any`        | Resolved value from task action |
